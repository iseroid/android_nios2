...
static void *eventHandlerArg = NULL;
 ...
 */
int16_t USB::initDevice( usb_device *device, int16_t configuration )
{
	uint8_t buf[4];
	int16_t rcode;
	...
	rcode = USB::getString( device, 0, 0, 4, (char *)buf );
	if( rcode < 0 ) {
		return rcode;
	}
 ...
 */
void USB::setEventHandler( usb_eventHandler *handler, void *cb_arg )
{
	eventHandler = handler;
	eventHandlerArg = cb_arg;
}
...
{
	eventHandler( device, event, eventHandlerArg );
}
 ...
 */
usb_device *USB::getDevice( uint8_t address )
{
	if( address > USB_NUMDEVICES+1 ) {
		return NULL;
	}
...
int8_t USB::dispatchPacket( uint8_t token, usb_endpoint *endpoint, uint16_t nakLimit )
{
...
void USB::poll()
{
	...
	case USB_ATTACHED_SUBSTATE_GET_DEVICE_DESCRIPTOR_SIZE:
		...
		if( rcode == 0 ) {
			...
			deviceTable[0].idVendor  = deviceDescriptor.idVendor;
			deviceTable[0].idProduct = deviceDescriptor.idProduct;
	...
	case USB_STATE_ADDRESSING:
		...
		for( i = 1; i < USB_NUMDEVICES; i++ ) {
			if( ! deviceTable[i].active ) {
				...
				deviceTable[i].control.maxPacketSize = deviceTable[0].control.maxPacketSize;
				deviceTable[i].idVendor  = deviceTable[0].idVendor;
				deviceTable[i].idProduct = deviceTable[0].idProduct;
 ...
 */
int8_t USB::getString( usb_device *device, uint8_t index, uint8_t languageId, uint16_t length, char *str )
{
 ...
 * @return number of bytes read, or error code in case of failure.
 */
int16_t USB::bulkRead( usb_device *device, uint16_t length, uint8_t *data, boolean poll )
{
 ...
 */
int8_t USB::write( usb_device * device, usb_endpoint *endpoint, uint16_t length, uint8_t *data )
{
	uint8_t rcode = 0;
	uint8_t retry_count;
 ...
 * @return number of bytes read, or error code in case of failure.
 */
int16_t USB::bulkWrite( usb_device *device, uint16_t length, uint8_t *data )
{
 ...
 */
int8_t USB::ctrlData(usb_device * device, boolean direction, uint16_t length, uint8_t * data)
{
	if( direction ) {
		// IN transfer
		...
		uint8_t * data)
{
	...
	rcode = USB::dispatchPacket( tokSETUP, &(device->control), USB_NAK_LIMIT );
...
//			serialPrintf("Data packet error: 0x%02x\n", rcode);
	...
	// Status stage.
	if( direction ) {
		rcode = USB::dispatchPacket( tokOUTHS, &(device->control), USB_NAK_LIMIT );
	} else {
		rcode = USB::dispatchPacket( tokINHS, &(device->control), USB_NAK_LIMIT );
	}
 ...
 */
int16_t USB::getConfigurationDescriptor(usb_device * device, uint8_t conf, uint16_t length, uint8_t * data)
{
 ...
 * @return 0 in case of success, error code otherwise
 */
int8_t USB::setAddress( usb_device * device, uint8_t address )
{
 ...
 */
int8_t USB::setConfiguration( usb_device * device, uint8_t configuration )
{
